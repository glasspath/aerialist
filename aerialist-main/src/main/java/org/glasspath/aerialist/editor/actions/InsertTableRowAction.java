...
public class InsertTableRowAction extends AbstractAction {
	...
	public InsertTableRowAction(EditorPanel<? extends EditorPanel<?>> context, TableCellView tableCellView, boolean above) {

		this.context = context;
		this.tableCellView = tableCellView;
		this.above = above;

		putValue(Action.NAME, above ? "Insert row above" : "Insert row below");
		putValue(Action.SHORT_DESCRIPTION, above ? "Insert row above" : "Insert row below");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (tableCellView.getParent() instanceof TableView) {
			...
			if (!above) {
				row = row + (tableCellView.getRowSpan() - 1) + 1;
			}
